...
2. **Anyone with the link and the board passphrase can edit.** No auth, no roles, no view-only mode. **All boards are passphrase-locked** (Phase 7.5 made this mandatory; there is no unlocked variant). The creator chooses the passphrase at create time; recipients need both the URL slug and the passphrase to decrypt. The "in the room" semantics: anyone who has both is in the room. There is no recovery flow — lost passphrase = lost board, by design (no accounts, no recovery email). Creating a *new* board additionally requires a site-wide `SITE_PASSWORD` (a single Fly secret, anti-bot) — this gate is for creation only; an existing slug's PATCH is open (slug entropy 9×10¹⁴ is the protection).
...
11. **Boards never round-trip plaintext through the server.** Since all boards are locked (invariant 2), the only thing the server ever stores or returns is `{ locked: true, ciphertext, iv, kdfSalt, kdfIters, updatedAt }`. Plaintext exists only in the browser memory of a session that has decrypted it. The server cannot enumerate contents, cannot search, cannot recover. Loss of the passphrase = loss of the board.
...
- **Per-board passphrase lock** (Phase 7.5 — resolved 2026-05-24, **mandatory**). All boards are encrypted. PBKDF2-SHA-256 with `kdfIters ≥ 200_000` derives a 256-bit key from `passphrase + kdfSalt` (16 random bytes per board, fresh on create). Content encrypted with AES-GCM-256 and a fresh 12-byte random IV per write. Server stores only `{ locked: true, ciphertext, iv, kdfSalt, kdfIters, updatedAt }`. Decryption happens entirely in the browser; the derived key is held in `sessionStorage` keyed by slug so reloads in the same tab don't re-prompt. There is no unlocked variant of a board.
- **Site-creation gate** (Phase 7.5 — resolved 2026-05-24). A single static `SITE_PASSWORD` (Fly secret) is required to create a new board. The client sends it as `X-Site-Password` on PATCH requests for slugs that don't yet exist; the server rejects unauthorized creation with 401. Existing-slug PATCH (edits) does NOT require the site password — slug entropy is the protection. This is for personal/private deployments; v2 may add per-user invites once accounts make sense (no plans for that yet).
- **Domain / hosting** (Phase 8 / launch — resolved 2026-05-24). **Fly.io** in `lhr`, app name `schedule-board`, single shared-CPU machine with a 1 GB persistent volume mounted at `/data` for the SQLite file. DNS at Cloudflare for `potterstudio.net`, grey-cloud (DNS-only) records pointing `schedule.potterstudio.net` at Fly's IPv4 + IPv6. Live at https://schedule.potterstudio.net/.
...
| 2026-05-24 | 8A | **Production deploy.** Single-process server collapse (`STATIC_DIR` env enables SPA serving + `/healthz` + `Accept: text/html` branch on `GET /b/:slug`). Dockerfile + fly.toml. Live at https://schedule.potterstudio.net/. | Phase 8A deploy slice split off so the app is live to dogfood before the launch-polish work (Phase 8B). |
| 2026-05-24 | 7.5 | **Per-board passphrase lock upgraded from optional → MANDATORY.** No unlocked variant. Adds a new `SITE_PASSWORD` Fly secret (anti-bot gate for *creation* only; existing-slug PATCH stays open). Invariant 2 rewritten; invariant 11 generalised ("all boards" not "locked boards"). New splash screen at `/` with two password fields (site + board); unlock screen at `/b/<slug>`. Per-tab key cache in `sessionStorage`. | Pre-deploy spot-check by the owner: the unconstrained-create surface lets bots spawn thousands of boards. Personal/private deployment doesn't need the "casual boards stay frictionless" middle ground — every board should be locked. Site password is the simplest viable anti-bot for a one-user deployment. |
