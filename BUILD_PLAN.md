...
- **Slugs: four random words + 4-digit suffix** (e.g. `oak-thread-helmet-tractor-7421`). Word list lives inline in `src/persistence/slug.ts` (~1500 adjectives + ~1500 nouns as plain TS arrays, no external dep). Combined entropy `1500⁴ × 10⁴ ≈ 5 × 10¹⁶` combinations — bots cannot enumerate. Collisions are tolerated by the backend (slug uniqueness is not enforced on the client; the unit test asserts shape, not global uniqueness). Router validation accepts any `[a-z0-9-]+` slug for forward-compat; only the *generator* enforces the four-word shape.
